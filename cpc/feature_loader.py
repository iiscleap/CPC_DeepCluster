...
def loadModel(pathCheckpoints, loadStateDict=True, updateConfig=None):
    ...
    for path in pathCheckpoints:
        ...
        if loadStateDict:     # Loads the weights here
...
def save_checkpoint_mod(model_state, criterion_state, optimizer_state, best_state,                        cpc_classifier, path_checkpoint):
   
   state_dict = {"gEncoder": model_state,
                 "cpcCriterion": criterion_state,
                 "optimizer": optimizer_state,
                 "best": best_state,
                 "cpc_classifier": cpc_classifier}
