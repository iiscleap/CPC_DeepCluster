...
class AudioBatchData(Dataset):
    ...
    def __init__(self,
        ...
        self.LabelsPath = Path(LabelsPath)
        self.sizeWindow = sizeWindow
        self.seqNames = [(s, self.dbPath / x,
                          self.LabelsPath / (x.split('.')[0].split('/')[-1]+'.pt'))
                         for s, x in seqNames]
    ...
    def clear(self):
        ...
        if 'seqLabel' in self.__dict__:
            del self.seqLabel
        if 'Psuedo_Data' in self.__dict__:
            del self.Psuedo_Data
        if 'Psuedo_seq_label' in self.__dict__:
            del self.Psuedo_seq_label
    ...
    def parseNextDataBlock(self):
        ...
        for speaker, seqName, seq, label_seq in self.nextData:
            ...
            sizeSeq = seq.size(0)
            labelSeqSize = label_seq.size(0)
            tmpData.append(seq)
            tmpPsuedo.append(label_seq)

            self.Psuedo_seq_label.append(
                self.Psuedo_seq_label[-1] + labelSeqSize)
        ...
        self.Psuedo_Data = torch.cat(tmpPsuedo, dim=0)
    ...
    def __getitem__(self, idx):
        ...
        # Code for getting index for psuedo labels
        start_idx = self.starting_index(idx)
        psuedo_labels = self.Psuedo_Data[start_idx:(
            (self.sizeWindow//160)+start_idx)].view(1, -1)
        ...
        return outData, label, psuedo_labels

    def starting_index(self, idx):

        downsampling = 160
        ptr = 0

        while idx > self.seqLabel[ptr]-1:
            ptr += 1

        counts = 0
        starting_point = self.seqLabel[ptr-1]-1

        while starting_point < idx:
            starting_point += downsampling
            counts += 1

        return self.Psuedo_seq_label[ptr - 1] + counts - 1
...
def loadFile(data):
    speaker, fullPath, fullLabelPath = data
    seqName = fullPath.stem
    seq = torchaudio.load(fullPath)[0].view(-1)
    labels = torch.load(fullLabelPath).view(-1)
    return speaker, seqName, seq, labels
...
def extractLength(couple):
    speaker, locPath, lab_path = couple
