...
class CPC_classification(nn.Module):

   def __init__(self,
                dimOutputAR):
      super(CPC_classification, self).__init__()
      self.classifier = nn.Linear(dimOutputAR,50)
      self.class_loss = nn.CrossEntropyLoss()

   def forward(self,cFeature,labels):
      cFeature = cFeature[:,:116]
      labels = labels[:,:,:116]
      labels = labels.contiguous().view(labels.size(0)*labels.size(2))
      output_class = self.classifier(cFeature)
      output_class = output_class.contiguous().view(-1,output_class.size(2))
      classloss = self.class_loss(output_class,labels)
      return classloss

#def getclassifier(dimOutputAR):
#   return CPC_classification(dimOutputAR)
...
#   classifier.load_state_dict(state_dict["classifier"],strict = False)
#   return classifier   
...
class CPCUnsupersivedCriterion(BaseCriterion):
    ...
    def forward(self, cFeature, encodedData, label):
        ...
        label = label.contiguous().view(label.size(0)*label.size(2))
        ...
        #output_class = self.classifier(cFeature)
        #output_class = output_class.contiguous().view(-1, output_class.size(2))
        #classloss = self.class_loss(output_class, label)
