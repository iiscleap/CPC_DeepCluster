...
#from cpc.criterion import getclassifier,loadclassifier
#
# This source code is licensed under the MIT license found in the
# LICENSE file in the root directory of this source tree.

def getclassifier(dimOutputAR):
   return cr.CPC_classification(dimOutputAR)

def loadclassifier(checkpoint,dimOutputAR):
   classifier = getclassifier(dimOutputAR)
   print("Loading the weights of the classifier")
   state_dict = torch.load(checkpoint,'cpu')
   classifier.load_state_dict(state_dict["cpc_classifier"],strict = False)
   return classifier
...
def trainStep(dataLoader,
    ...
    cpcModel.train()
    cpcCriterion.train()
    cpc_classifier.train()
    # include classification model here
    ...
    for step, fulldata in enumerate(dataLoader):
        batchData, label, psuedo_labels = fulldata
        n_examples += batchData.size(0)
        batchData = batchData.cuda(non_blocking=True)
        label = label.cuda(non_blocking=True)
        psuedo_labels = psuedo_labels.cuda(non_blocking=True)
        c_feature, encoded_data, label = cpcModel(batchData, label)
        allLosses, allAcc = cpcCriterion(
            c_feature, encoded_data, psuedo_labels)
        loss = cpc_classifier(c_feature,psuedo_labels)
        totLoss = allLosses.sum() + weight * loss
        ...
        if "TrainClassLoss" not in logs:
            logs["TrainClassLoss"] = np.zeros((1,))

        iter += 1
        logs["locLoss_train"] += (allLosses.mean(dim=0)).detach().cpu().numpy()
        logs["locAcc_train"] += (allAcc.mean(dim=0)).cpu().numpy()

        logs["TrainClassLoss"] += (loss).detach().cpu().numpy()
...
def valStep(dataLoader,
    ...
    for step, fulldata in enumerate(dataLoader):
        ...
        if "valClassLoss" not in logs:
            logs["valClassLoss"] = np.zeros((1,))

        logs["valClassLoss"] += loss.cpu().numpy()
        iter += 1
...
def run(trainDataset,
    ...
    if "locAcc_val" in logs and len(logs["locAcc_val"]) > 0:
        ...
        if count_inverse(valAccuracyList) > patience:
            print(
                f"The patience={patience} has been reached, early stopping activated. Stopped!")
            return
    else:
        valAccuracyList = []

    if "locAcc_train" in logs and len(logs["locAcc_train"]) > 0:
        TrainAccuracyList = [100*np.mean(ls) for ls in logs['locAcc_train']]
    else:
        TrainAccuracyList = []

    if "TrainClassLoss" in logs and len(logs["TrainClassLoss"]) > 0:
        train_Class_loss = [ls[0] for ls in logs["TrainClassLoss"]]
    else:
        train_Class_loss = []

    if "valClassLoss" in logs and len(logs["valClassLoss"]) > 0:
        val_Class_loss = [ls[0] for ls in logs["valClassLoss"]]
    else:
        val_Class_loss = []
    ...
    for epoch in range(startEpoch, nEpoch):
        ...
        # In both trainStep and valStep include classification class
        locLogsTrain = trainStep(trainLoader, cpcModel, cpcCriterion,cpc_classifier,
                                 optimizer, scheduler, logs["logging_step"], weight)

        locLogsVal = valStep(valLoader, cpcModel, cpcCriterion,cpc_classifier)
        ...
        currentTrainAccuracy = float(locLogsTrain["locAcc_train"].mean())
        TrainAccuracyList.append(100*currentTrainAccuracy)

        val_Class_loss.append(locLogsVal["valClassLoss"][0])
        train_Class_loss.append(locLogsTrain["TrainClassLoss"][0])
        ...
        if pathCheckpoint is not None \
            ...
            cpc_classifierDict = fl.get_module(cpc_classifier).state_dict()

            fl.save_checkpoint_mod(modelStateDict, criterionStateDict,
                               optimizer.state_dict(), bestStateDict,cpc_classifierDict,
            ...
            with open(os.path.join(os.path.dirname(pathCheckpoint), "valAcc_info.txt"), 'w') as file:
                outLines = [f"Epoch {ep} : {acc}" for ep, acc in enumerate(
                    valAccuracyList)] + [f"Best valAcc : checkpoint_{np.argmax(valAccuracyList)}"]
                file.write("\n".join(outLines))

            with open(os.path.join(os.path.dirname(pathCheckpoint), "TrainAcc_info.txt"), 'w') as file:
                outLines = [f"Epoch {ep} : {acc}" for ep, acc in enumerate(
                    TrainAccuracyList)]
                file.write("\n".join(outLines))

            with open(os.path.join(os.path.dirname(pathCheckpoint), "valLoss_info.txt"), 'w') as file:
                outLines = [f"Epoch {ep} : {loss}" for ep,
                            loss in enumerate(val_Class_loss)]
                file.write("\n".join(outLines))

            with open(os.path.join(os.path.dirname(pathCheckpoint), "TrainLoss_info.txt"), 'w') as file:
                outLines = [f"Epoch {ep} : {loss}" for ep,
                            loss in enumerate(train_Class_loss)]
                file.write("\n".join(outLines))
        ...
        if earlyStopping:
            if count_inverse(valAccuracyList) > patience:
                ...
                with open(os.path.join(os.path.dirname(pathCheckpoint), "earlyStopping.txt"), 'a') as file:
                    file.write(
                        f"The patience={patience} has been reached, early stopping activated. Stopped at epoch {epoch}!\n")
                break
...
def main(args):
    ...
    # Introduce code to load classification layer ; write in try except style
    # load state dict for these in try except style
    
    try:
       cpc_classifier = loadclassifier(args.load[0],args.hiddenGar)
    except:
       cpc_classifier = getclassifier(args.hiddenGar)
    ...
    # Optimizer
    # Also add params of classification layer
    g_params = list(cpcCriterion.parameters()) + list(cpcModel.parameters()) + list(cpc_classifier.parameters())
   ...
   # cpcCriterion = torch.nn.DataParallel(cpcCriterion,
    ...
    run(trainDataset,
        ...
        cpcModel,
        cpcCriterion,
        cpc_classifier,				# need to include classification module
        args.nEpoch,
        args.pathCheckpoint,
        optimizer,
        scheduler,
...
def parseArgs(argv):
    ...
    add_Labels = parser.add_argument_group('Psuedo Labels')
    add_Labels.add_argument('--LabelsPath', type=str, required=True,
                            help="Path to Psuedo Labels")
    weight = parser.add_argument_group('Loss Weighting')
    weight.add_argument('--alpha_val', type=int, default=1,
                        help="Weights the CPC and CLusterLoss")
    ...
    group_save.add_argument('--patience', type=int, default=10)
